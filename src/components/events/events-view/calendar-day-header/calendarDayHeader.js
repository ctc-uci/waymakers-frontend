...
const CalendarDayHeader = ({ goToPrev, goToNext, dayInfo }) => {
  const renderDayHeader = () => {
    const prevButton = () => <button className="cal-btn prev-button" type="button" onClick={goToPrev} aria-label="previous"><IconIo.IoIosArrowBack size={20} /></button>;
    const nextButton = () => <button className="cal-btn next-button" type="button" onClick={goToNext} aria-label="next"><IconIo.IoIosArrowForward size={20} /></button>;
