...
const Overview = (prop) => {
  ...
  //   const startDate = `${startMonth} ${event.startDay}${suffixDict[event.startDay % 10] ===
  // undefined ? 'th' : suffixDict[event.startDay % 10]}`;
  //   const endDate = `${endMonth} ${event.endDay}${suffixDict[event.endDay % 10] ===
  // undefined ? 'th' : suffixDict[event.endDay % 10]}`;
  //   return (
  //     <>
  ...
  //           <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .4
  // 96-.868L8 8.71V3.5z" />
    ...
    <div className="event-details">
      ...
      eventInfo.length === 1 && (
        <div className="overview-container">
          <p className="event-date">January 15</p>
          <div className="event-info">
            <img className="event-info-icon" src={clockIcon} alt="time" />
            <span className="event-info-detail">{eventInfo[0].startTime}</span>
          </div>
          <div className="event-info">
            <img className="event-info-icon" src={locationPinIcon} alt="location" />
            <span className="event-info-detail">{eventInfo[0].location}</span>
          </div>
          {/* <p className="event-description">{eventInfo[0].description}</p> */}
